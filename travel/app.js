...
const OPTION_MEDIA={
'Grand Central Terminal':{wiki:'Grand_Central_Terminal',intro:'紐約最漂亮、也最實用的交通建築之一。主大廳、星空天花板與四面鐘很值得看，從 LIC 搭 7 Train 幾站即可抵達。',map:'Grand Central Terminal, New York, NY',official:'https://www.grandcentralterminal.com/'},
...
'Gantry Plaza State Park':{wiki:'Gantry_Plaza_State_Park',intro:'LIC 河岸最實用的免費景點之一，正對 Midtown skyline，日落與夜景都很漂亮，也很適合安排在低強度時段。',map:'Gantry Plaza State Park, Long Island City, NY',official:'https://parks.ny.gov/parks/gantryplaza'},
...
'Korean Parade & Festival':{wiki:'Koreatown,_Manhattan',intro:'10/3 正逢 Korean Parade 與 Koreatown Festival。這不是常態景點，而是旅程期間限定的街區體驗。',map:'Koreatown, Manhattan, New York, NY',official:'https://kaagny.org/'},
'New York Film Festival':{wiki:'New_York_Film_Festival',intro:'Lincoln Center 的代表性電影節，2026 年檔期與旅程日期重疊。若有喜歡的導演或首映，是很有紐約城市感的夜間活動。',map:'Film at Lincoln Center, New York, NY',official:'https://www.filmlinc.org/nyff/'},
...
'Brooklyn Bridge walk':{wiki:'Brooklyn_Bridge',intro:'經典紐約步行體驗，但步行量不低。若要走，從 Brooklyn 往 Manhattan 方向景觀較直覺；體力有限時可只在 DUMBO 河岸看橋。',map:'Brooklyn Bridge Pedestrian Walkway, New York, NY',official:'https://www.nyc.gov/html/dot/html/infrastructure/brooklyn-bridge.shtml'},
...
function renderOptions(cat='全部'){
  ...
  grid.innerHTML=list.map(o=>{const m=OPTION_MEDIA[o.name]||{};return `<article class="card option option-rich" data-wiki="${m.wiki||''}">
    ...
    <div class="option-content">
      ...
      <div class="meters"><div><span>體力友善</span>${dots(6-o.effort)}</div><div><span>雨天友善</span>${dots(o.indoor)}</div><div><span>步調友善</span>${dots(o.mom)}</div></div>
